...
int firsttime = 1;
int count_p = 0;
static GLint array_polygon[MAX];
static GLsizei count_polygon[MAX];
static GLvoid* array_polygon_e[MAX];
...
void state_2_poly(int start, int times, int* test) {
    //static GLubyte tmp[] = {};
    short int start_tmp = start;
    test = (int*)malloc(sizeof(int) * times);
    ...
    if (firsttime == 1) {
        array_polygon[count_p] = start_tmp;
        count_polygon[count_p] = times;
        array_polygon_e[count_p] = test;
        count_p++;
    }
...
void state_3() {
    glMultiDrawArrays(GL_POLYGON, array_polygon, count_polygon, count_p);
}

void state_4() {
    glMultiDrawElements(GL_POLYGON, count_polygon, GL_UNSIGNED_INT, array_polygon_e, count_p);
}
